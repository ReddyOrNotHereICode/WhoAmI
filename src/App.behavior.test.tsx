...
describe("Theme toggling", () => {
  it("toggles between light and dark mode when the icon is clicked", async () => {
    ...
    expect(
      toggleBtn.querySelector('[data-testid="FaMoon"]'),
    ).toBeInTheDocument();
    await userEvent.click(toggleBtn);
    expect(
      screen
        .getByLabelText(/toggle dark mode/i)
        .querySelector('[data-testid="FaSun"]'),
    ).toBeInTheDocument();
  });
  ...
  it("toggles from light to dark and back to light, covering both branches", async () => {
    ...
    await userEvent.click(toggleBtn);
    expect(
      screen.getByLabelText(/toggle dark mode/i).querySelector("svg"),
    ).toBeInTheDocument();
    await userEvent.click(screen.getByLabelText(/toggle dark mode/i));
    expect(
      screen.getByLabelText(/toggle dark mode/i).querySelector("svg"),
...
describe("Contact popover", () => {
  ...
  it("closes when clicking away", async () => {
    ...
    expect(
      screen.queryByText(/red.parker.red@gmail.com/i),
